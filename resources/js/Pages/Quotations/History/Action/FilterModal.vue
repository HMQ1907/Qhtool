...
const { props } = usePage();
const emit = defineEmits(["close", "update-quotations"]);
...
const filter = () => {
    form.get(route("quotation.history.index"), {
        onStart: () => {
            closeModal();
        },
        onSuccess: (data) => {
            emit("update-quotations", data.quotations);
        },
    });
};
</script>
